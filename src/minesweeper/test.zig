...
test "Number uncover" {
    ...
    minesweeper.toggle_flag(&game_state, .{ .x = 0, .y = 0 });
    minesweeper.toggle_flag(&game_state, .{ .x = 0, .y = 2 });
    minesweeper.uncover(&game_state, .{ .x = 1, .y = 1 });

    try expect(game_state.event_history[0] == minesweeper.GameEventTag.discover_single);
    ...
    try expect(!game_state.is_ended);
}
